  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userData.role === 'peserta') {
        ...
        console.warn("Login berhasil, namun role bukan peserta:", userData.role);
      }

    } catch (err: unknown) { // <-- Perbaikan di sini: eksplisitkan 'unknown'
      ...
      if (axios.isAxiosError(err)) { // Cek apakah ini AxiosError
        const responseData = err.response?.data; // responseData bisa undefined, jadi gunakan optional chaining

        if (err.response?.status === 401 || err.response?.status === 403) { // Gunakan optional chaining
          ...
          setError(`Error: ${err.response?.status} - ${err.response?.statusText}`); // Gunakan optional chaining
        }
      } else if (err instanceof Error) { // Jika ini adalah Error standar JavaScript
        setError(`Terjadi kesalahan: ${err.message}`);
      } else { // Jika tipe error lain yang tidak teridentifikasi
