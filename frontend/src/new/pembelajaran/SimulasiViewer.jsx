...
export default function SimulasiViewer({ simulationId }) {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const res = await api.get(`/questions?simulation_id=${simulationId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setQuestions(res.data);
      } catch (err) {
        alert('Gagal load soal simulasi');
      }
    };

    const loadResult = async () => {
      try {
        const res = await api.get(`/simulations/${simulationId}/result`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.data) {
          setResult(res.data);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await api.post('/simulations/submit', payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setResult(res.data);
    } catch (err) {
      ...
      <div>
        ...
        <ul>
          {result.details.map((r, i) => (
            <li key={i}>
              <b>Soal {i + 1}:</b> {r.is_correct ? '✅ Benar' : '❌ Salah'}<br />
              Jawaban Benar: {r.correct_option}<br />
              <i>{r.explanation}</i>
            </li>
          ))}
        </ul>
  ...
  return (
    <div>
      <h4>Soal {current + 1} / {questions.length}</h4>
      <p>{q.question_text}</p>
