...
export default function HasilSimulasi() {
  ...
  const loadResults = async () => {
    try {
      ...
      if (err.response?.status === 403) {
        ...
        setError('Gagal memuat hasil simulasi: ' + (err.response?.data?.message || err.message));
      }
      setLoading(false);
    }
  };
  ...
  const getCEFRColor = (cefr) => {
    switch (cefr) {
      ...
      case 'B2': return '#B6252A';
      case 'B1': return '#17a2b8';
      case 'A2-B1': return '#fd7e14';
      case 'A2': return '#dc3545';
          ...
          return (
            <div key={opt} style={{ marginBottom: "0.5rem" }}>
              <div style={{ 
                ...
                <span><strong>{opt.toUpperCase()}.</strong> {answer.question[`option_${opt}`]}</span>
              </div>
            </div>
          );
        })}
  ...
  const renderGroupQuestion = (group) => {
    ...
    const startOrder = children.length > 0 ? children[0].question.order_number : 0;
    const endOrder = children.length > 0 ? children[children.length - 1].question.order_number : startOrder;
          ...
          {children.map((answer) => (
            ...
            >
              <h6 style={{ 
                ...
                Soal #{answer.question.order_number}. {answer.question.question_text}
              </h6>
  ...
  const { simulation, answers, group_parents, scores, raw_scores, time_spent, score_interpretation } = results;
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <p style={{ margin: 0, opacity: 0.9 }}>
            Diselesaikan pada: {new Date(simulation.finished_at).toLocaleString('id-ID')}
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            Score Range: {score_interpretation.total_range} | 
            Total Time: {formatTime(time_spent.total)}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h4 style={{ margin: '0 0 0.5rem 0', color: '#495057', fontWeight: '600' }}>
            ...
            <div style={{ fontSize: '14px', color: '#6c757d', marginBottom: '0.5rem' }}>
              Raw Score: {raw_scores.listening.correct}/{raw_scores.listening.total} ({raw_scores.listening.percentage}%)
            </div>
            <div style={{ fontSize: '12px', color: '#6c757d' }}>
              Time: {formatTime(time_spent.listening)} | Range: {score_interpretation.listening_range}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: '#6c757d', marginBottom: '0.5rem' }}>
              Raw Score: {raw_scores.structure.correct}/{raw_scores.structure.total} ({raw_scores.structure.percentage}%)
            </div>
            <div style={{ fontSize: '12px', color: '#6c757d' }}>
              Time: {formatTime(time_spent.structure)} | Range: {score_interpretation.structure_range}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '14px', color: '#6c757d', marginBottom: '0.5rem' }}>
              Raw Score: {raw_scores.reading.correct}/{raw_scores.reading.total} ({raw_scores.reading.percentage}%)
            </div>
            <div style={{ fontSize: '12px', color: '#6c757d' }}>
              Time: {formatTime(time_spent.reading)} | Range: {score_interpretation.reading_range}
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '0.5rem' }}>
            {Object.entries(score_interpretation.cefr_mapping).map(([level, range]) => (
              <div 
                key={level}
                style={{
                  padding: '0.5rem',
                  borderRadius: '4px',
                  textAlign: 'center',
                  backgroundColor: level === scores.cefr_level ? getCEFRColor(level) : '#e9ecef',
                  color: level === scores.cefr_level ? 'white' : '#495057',
                  fontWeight: level === scores.cefr_level ? 'bold' : 'normal'
                }}
              >
                <div style={{ fontSize: '14px', fontWeight: '600' }}>{level}</div>
          ...
          return (
            <div key={module} style={{
              ...
              <h3 style={{ 
                ...
                {module === 'structure' ? 'Structure & Written Expression' : 
                 module.charAt(0).toUpperCase() + module.slice(1)} Comprehension
              </h3>
              ...
              <div style={{ 
                ...
                <strong>
                  Score: {scores[module]} | 
                  Raw Score: {raw_scores[module].correct}/{raw_scores[module].total} ({raw_scores[module].percentage}%) | 
                  Time: {formatTime(time_spent[module])}
                </strong>
              </div>
              ...
              <div style={{ 
                ...
                {(() => {
                  ...
                  organized.groups.forEach(group => {
                    const firstChildOrder = group.children.length > 0 
        ...
        <div style={{
          ...
          <strong>Disclaimer:</strong> Ini adalah simulasi TOEFL ITP untuk latihan. 
          Skor ini tidak dapat digunakan untuk keperluan resmi. 
          Untuk tes resmi, silakan daftar di pusat tes TOEFL ITP yang diakui ETS.
        </div>
