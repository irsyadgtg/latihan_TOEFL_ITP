...
export default function PageViewer({ 
  ...
  const checkPageStatus = async (pageId) => {
    ...
    try {
      ...
      const nextRequiredPage = pageCanAccess ? null : getNextRequiredPage(pageList, completedPageIds);
      ...
      console.log('PageViewer: Access check results:', {
        ...
        currentPageTitle: pageList.find(p => p.id === pageId)?.title || 'Unknown'
      });
      ...
      if (!pageCanAccess && nextRequiredPage) {
        console.log('PageViewer: Access denied, next required page:', nextRequiredPage);
      }
  ...
  const handleNext = async () => {
    ...
    if (role === 'peserta' && page && !isCompleted) {
      const shouldComplete = window.confirm(
        `Halaman "${page.title}" belum ditandai selesai.\n\nTandai selesai dan lanjut ke halaman berikutnya?`
      );
      ...
      if (shouldComplete) {
        try {
          ...
          console.log('PageViewer: Fresh progress data after complete:', freshProgressData);
    ...
    if (role === 'peserta' && page && isCompleted) {
      try {
        ...
        const freshProgressData = freshProgressRes.data;
        console.log('PageViewer: Fresh progress data for navigation:', freshProgressData);
        
        setProgressData(freshProgressData);
        
        if (onNext) {
          console.log('PageViewer: Calling onNext with fresh progress data (completed page)');
          onNext(freshProgressData);
        }
      ...
      return (
        <img 
          ...
          onError={(e) => {
            ...
            e.target.parentNode.innerHTML += '<p style="color: #dc3545;">Gagal memuat gambar</p>';
          }}
        />
      );
    }
      ...
      return (
        <audio 
          ...
          onError={(e) => {
            ...
            e.target.parentNode.innerHTML += '<p style="color: #dc3545;">Gagal memuat audio</p>';
          }}
        >
      ...
      return (
        <video 
          ...
          onError={(e) => {
            ...
            e.target.parentNode.innerHTML += '<p style="color: #dc3545;">Gagal memuat video</p>';
          }}
        >
      ...
      <div style={{ 
          ...
          {shouldShowQuizButton() && (
            <>
              ...
              <button
                ...
                style={{
                  backgroundColor: showQuiz ? "#B6252A" : 
                                  (role === 'peserta' && !progressData.can_access_quiz) ? "#dc3545" : "#6c757d",
                  color: "white",
                  border: "none",
                  padding: "0.5rem",
                  borderRadius: "3px",
                  cursor: (role === 'peserta' && !progressData.can_access_quiz) ? "not-allowed" : "pointer",
                  textAlign: "center",
                  fontSize: "0.8rem",
                  fontWeight: "600",
                  width: "100%"
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ margin: "0" }}>Silakan pilih unit atau tambah halaman baru.</p>
          </div>
        </div>
      </div>
    );
            ...
            pageList.map((p, index) => {
              ...
              return (
                <button
                  ...
                  disabled={role === 'peserta' && !canAccess && currentPageId !== p.id}
                  style={{
                    backgroundColor: currentPageId === p.id ? "#B6252A" : 
                                    pageCompleted ? "#28a745" : "#e9ecef",
                    color: (currentPageId === p.id || pageCompleted) ? "white" : "#495057",
                    border: "none",
                    padding: "0.5rem",
                    borderRadius: "3px",
                    cursor: "pointer",
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              Halaman {pageList.findIndex(p => p.id === page.id) + 1} dari {pageList.length}
            </div>
          </div>
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <button 
              ...
              style={{
                backgroundColor: (onNext && (role !== 'peserta' || canAccess)) ? "#B6252A" : "#6c757d",
                color: "white",
                border: "none",
                padding: "0.5rem 1rem",
                borderRadius: "4px",
                cursor: (onNext && (role !== 'peserta' || canAccess)) ? "pointer" : "not-allowed",
                fontSize: "0.9rem"
              }}
            >
              Selanjutnya →
