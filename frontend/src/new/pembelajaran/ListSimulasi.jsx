...
export default function ListSimulasi() {
  ...
  const loadSimulations = async () => {
    try {
      const res = await api.get('/simulations/completed?simulation_set_id=1', {
      ...
      setError('Gagal memuat daftar simulasi: ' + (err.response?.data?.message || err.message));
      setLoading(false);
    }
  };

  const viewResult = (simulationId) => {
    navigate(`/simulasi/hasil/${simulationId}`);
  };
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{
        ...
        <h2 style={{ margin: 0, fontWeight: '600' }}>Riwayat Hasil Simulasi TOEFL ITP</h2>
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: '200px' }}>
                ...
                <div style={{ fontSize: '14px', color: '#6c757d' }}>
                  Tanggal: {new Date(simulation.finished_at).toLocaleDateString('id-ID', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                    hour: '2-digit',
