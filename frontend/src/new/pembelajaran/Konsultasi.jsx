...
export default function Konsultasi() {
  ...
  useEffect(() => {
    checkConsultationAccess(); // ← TAMBAH INI
    loadSidebarData();
    loadUnits();
    
    // Refresh data every 30 seconds for real-time availability updates
  ...
  const checkConsultationAccess = async () => {
    ...
    try {
      const response = await api.get('/consultations/check-access');
  ...
  const loadSidebarData = async () => {
    try {
      ...
      if (role === 'peserta') {
        const response = await api.get('/consultations/instructors');
        ...
        const response = await api.get('/consultations/students');
  ...
  const loadConsultation = async () => {
    ...
    try {
      setLoading(true);
      const response = await api.get(`/consultations/${targetId}`);
  ...
  const loadUnits = async () => {
    try {
      const response = await api.get('/consultation-units');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      let response;
      if (isInstructor) {
        response = await api.post(`/consultations/${consultation.id}/instructor-message`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        response = await api.post(`/consultations/${targetId}/messages`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const handleEndSession = async () => {
    ...
    try {
      const response = await api.post(`/consultations/${consultation.id}/end-session`);
  ...
  const renderReference = (msg) => {
    ...
    const handleReferenceClick = () => {
      if (msg.reference_modul && msg.reference_unit_number) {
        const targetUrl = `/materi/${msg.reference_modul}?unit=${msg.reference_unit_number}`;
        navigate(targetUrl);
      } else if (msg.reference_modul) {
        const targetUrl = `/materi/${msg.reference_modul}`;
        navigate(targetUrl);
      }
    };
  ...
  // NEW: Render access denied message for chat area
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center', maxWidth: '500px' }}>
          ...
          <h3 style={{ margin: '0 0 1rem 0', color: '#B6252A', fontSize: '1.5rem' }}>
          ...
          {instructors.length === 0 ? (
            ...
            instructors.map(instructor => {
              ...
              return (
                <div
                  key={instructor.id}
                  onClick={() => canClick && handleUserSelect(instructor.id, instructor.is_available)}
                  style={{
                    padding: '1rem',
                    ...
                    transition: 'all 0.2s ease',
                    borderLeft: isActive ? '4px solid #B6252A' : '4px solid transparent',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{
                      ...
                      height: '40px',
                      borderRadius: '50%',
                      backgroundColor: isActive ? '#B6252A' : (canClick ? '#28a745' : '#6c757d'),
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      fontSize: '1rem',
        ...
        <div style={{ display: 'grid', gap: '0.5rem' }}>
          {studentConsultations.length === 0 ? (
            ...
            studentConsultations.map(consultation => {
              ...
              const userName = consultation.student?.username || consultation.student?.name || 'Student';
                      ...
                      <div style={{
                        ...
                        <span style={{ fontSize: '0.7rem', color: '#999' }}>
                          {consultation.latest_message && 
                            new Date(consultation.latest_message.created_at).toLocaleTimeString('id-ID', {
                              hour: '2-digit',
                              minute: '2-digit'
                            })
                          }
    ...
    <div style={{ 
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '600', color: '#495057' }}>
          ...
          {role === 'peserta' && (
            <p style={{ 
              ...
              Status diperbarui real-time setiap 30 detik
            </p>
          )}
        </div>
      ...
      <div style={{ 
        ...
        ) : !hasConsultationAccess && role === 'peserta' ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '600' }}>
                      {instructor.name} {isInstructor ? '(Student)' : '(Instruktur)'}
                    </h3>
                    <div style={{ fontSize: '0.9rem', color: '#666', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{
                        ...
                        fontSize: '0.75rem',
                        fontWeight: '600',
                        backgroundColor: consultation ? getStatusColor(consultation.status) : '#28a745',
                        color: 'white'
                      }}>
                        {consultation ? getStatusText(consultation.status) : 'Siap Konsultasi'}
                      </span>
                    </div>
                  </div>
                </div>
                      ...
                      <div style={{
                        ...
                        <div style={{ marginBottom: '0.5rem' }}>
                          <strong style={{ fontSize: '0.9rem' }}>
                            {msg.sender ? (msg.sender.username || msg.sender.name) : 'System'}
                          </strong>
                          <span style={{ 
                            fontSize: '0.8rem', 
                            opacity: 0.7,
            ...
            <div style={{
                      ...
                      REFERENCE: {
                        reference.unit_number 
                          ? reference.unit_number === '0' 
                            ...
                            : `Unit ${reference.unit_number} - ${reference.modul.toUpperCase()}`
              ...
              {showReference && canSendMessage && (
                <div style={{
                  ...
                  <h4 style={{ margin: '0 0 1rem 0', fontSize: '1rem' }}>REFERENCE: Tambah Reference</h4>
                  ...
                  {role === 'peserta' && (
                    <div style={{
                      ...
                      <strong>Info:</strong> Hanya unit yang sudah terbuka dari rencana belajar Anda yang dapat dipilih sebagai referensi.
                    </div>
                  )}
                  ...
                  <div style={{ display: 'grid', gap: '1rem' }}>
                      ...
                      >
                        ...
                        {Object.keys(units).map(modulName => (
                          <option key={modulName} value={modulName}>
                            {modulName.charAt(0).toUpperCase() + modulName.slice(1)}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    {reference.modul && units[reference.modul] && (
                      <div>
                        ...
                        >
                          <option value="">Pilih unit (kosongkan untuk semua unit)...</option>
                          {units[reference.modul].map(unitNum => (
                            <option key={unitNum} value={unitNum}>
                              {unitNum === 0 ? 'Unit Overview' : `Unit ${unitNum}`}
                            </option>
                          ))}
                        </select>
                        
                        {/* Show accessible units info for students */}
                        {role === 'peserta' && units[reference.modul] && (
                          <div style={{
                            marginTop: '0.5rem',
                            fontSize: '0.8rem',
                            color: '#666'
                          }}>
                            Unit yang dapat diakses: {units[reference.modul].join(', ')}
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
                    ...
                    <button
                      ...
                      style={{
                        padding: '0.5rem 1rem',
                        backgroundColor: reference.modul ? '#28a745' : '#6c757d',
                        color: 'white',
                        border: 'none',
                        borderRadius: '4px',
              ...
              <div style={{ 
                ...
                <button
                  ...
                  disabled={!canSendMessage || (!newMessage.trim() && !attachment)}
                  style={{
                    padding: '0.75rem 1.5rem',
                    backgroundColor: (canSendMessage && (newMessage.trim() || attachment)) ? '#B6252A' : '#6c757d',
                    color: 'white',
                    border: 'none',
                    borderRadius: '20px',
                    cursor: (canSendMessage && (newMessage.trim() || attachment)) ? 'pointer' : 'not-allowed',
                    fontWeight: '600'
                  }}
                >
                  Kirim
