...
export default function KelolaSimulasi() {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      const res = await api.get(`/questions?modul=${modul}&simulation_set=1`);
      console.log("Fetched questions:", res.data);
      setQuestions(res.data || []);
      
      // FETCH 2: All simulation questions for global count
      const allRes = await api.get(`/questions?simulation_set=1`);
  ...
  const fetchSimulationSet = async () => {
    try {
      const res = await api.get('/simulation-sets/1');
  ...
  const toggleActivation = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post(`/simulation-sets/${simulationSet.id}/toggle-active`);
      ...
      setError("Gagal mengubah status aktivasi: " + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const getGlobalTotalCount = () => {
    const allSimulationQuestions = allQuestions.filter((q) => 
    ...
    console.log('Global TOEFL count debug (FIXED):', {
      ...
      byModul: {
        listening: allSimulationQuestions.filter(q => q.modul === 'listening').length,
        structure: allSimulationQuestions.filter(q => q.modul === 'structure').length,
        reading: allSimulationQuestions.filter(q => q.modul === 'reading').length,
      },
  ...
  const getTOEFLStandard = () => {
    ...
    const totalStandard = Object.values(standards).reduce((sum, count) => sum + count, 0);
    return { standards, totalStandard };
  };

  const getRecommendedOrderNumber = () => {
    const currentModulQuestions = questions.filter((q) => 
      q.modul === modul && 
      q.correct_option !== null && 
      q.correct_option !== "" &&
      q.order_number
    ...
    const maxOrder = Math.max(...currentModulQuestions.map(q => q.order_number));
    return maxOrder + 1;
  };

  const getAvailableOrderNumbers = (excludeId = null, isGroup = false, isCreating = false) => {
    ...
    if (isCreating) {
      ...
      const usedOrderNumbers = realQuestions.map(q => q.order_number).sort((a, b) => a - b);
      ...
      usedOrderNumbers.forEach(orderNum => {
        availableOrders.push({
          ...
          label: orderNum === currentPosition ? `${orderNum} (posisi saat ini)` : `${orderNum}`
        });
      });
    }
    ...
    console.log('Available orders debug:', {
      modul,
      isCreating,
      availableOrders: availableOrders.map(o => ({value: o.value, label: o.label, isRecommended: o.isRecommended})),
      currentPosition
    });
  ...
  const validateGroupForm = () => {
    ...
    const groupValidation = validateGroupSize(form.children.length, form.group_order_number);
  ...
  const createIndividualQuestion = () => {
    ...
    const recommendedOrder = availableOrders.find(o => o.isRecommended)?.value || getRecommendedOrderNumber();
  ...
  const editGroupQuestion = (q) => {
    ...
    const groupOrderNumber = existingChildren.length > 0 
      ...
      <div style={{
        ...
        {simulationSet && (
          <div style={{
            ...
            marginTop: "1rem",
            borderRadius: "4px",
            border: `1px solid ${simulationSet.is_active ? "#c3e6cb" : "#f5c6cb"}`,
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center"
          }}>
            <div>
              <strong>Status Simulasi:</strong> {simulationSet.is_active ? "AKTIF" : "TIDAK AKTIF"}
              <br />
              <small>
            ...
            <button
              ...
              disabled={loading}
              style={{
                backgroundColor: simulationSet.is_active ? "#dc3545" : "#28a745",
                color: "white",
                border: "none",
                padding: "0.75rem 1.5rem",
                borderRadius: "4px",
            ...
            >
              {loading ? "Loading..." : (simulationSet.is_active ? "Nonaktifkan" : "Aktifkan")}
            </button>
          </div>
        )}
        ...
        {simulationSet && simulationSet.is_active && (
          <div style={{
            ...
            <div>
              <strong>Mode Read-Only:</strong> Simulasi sedang aktif. Semua fitur pengelolaan soal dinonaktifkan. 
              Nonaktifkan simulasi terlebih dahulu untuk dapat mengelola soal.
            </div>
          </div>
        )}
            ...
            >
              ...
              {index === modulIndex && (
                <span style={{ marginLeft: "0.5rem", fontSize: "0.8rem" }}>
                  ({getModulSoalCount(modulName)}/{getTOEFLStandard().standards[modulName]})
                </span>
              )}
            </button>
          ))}
        ...
        <div style={{ 
          ...
          <button 
            ...
            style={{
              backgroundColor: (loading || (simulationSet && simulationSet.is_active)) ? "#6c757d" : "#007bff",
              color: "white",
              border: "none",
              padding: "0.5rem 1rem",
              borderRadius: "4px",
              cursor: (loading || (simulationSet && simulationSet.is_active)) ? "not-allowed" : "pointer",
              fontWeight: "500"
            }}
            title={simulationSet && simulationSet.is_active ? "Simulasi sedang aktif - tidak dapat mengelola soal" : ""}
          >
          ...
          <button
            ...
            disabled={loading || (simulationSet && simulationSet.is_active)}
            style={{
              backgroundColor: (loading || (simulationSet && simulationSet.is_active)) ? "#6c757d" : "#28a745",
              color: "white",
              border: "none",
              padding: "0.5rem 1rem",
              borderRadius: "4px",
        ...
        <div style={{ 
          ...
          {(() => {
            ...
            return (
              <>
                <strong>Modul {modul}:</strong> {currentModulCount}/{standardForThisModul} soal | 
                <strong> Individual:</strong> {getIndividualQuestions().length} | 
                <strong> Grup:</strong> {getGroupQuestions().length} | 
                <strong> Subsoal:</strong> {getGroupQuestions().reduce((sum, g) => sum + getGroupChildren(g.id).length, 0)} <br />
                <strong>Total TOEFL ITP:</strong> {globalTotal}/{totalStandard} soal 
                <span style={{ color: globalTotal === totalStandard ? "#28a745" : "#dc3545" }}>
                  {globalTotal === totalStandard ? " Lengkap" : ` (kurang ${totalStandard - globalTotal})`}
                </span>
              </>
            );
          })()}
          ...
          <div style={{ 
            ...
            {getAllQuestionsSorted().map((item) => (
              <div
                key={item.id}
                style={{
                  border: item.type === "group" ? "2px solid #28a745" : "2px solid #007bff",
                  borderRadius: "4px",
                  padding: "1rem",
                  backgroundColor: item.type === "group" ? "#f8fff9" : "#f0f8ff"
                }}
              >
                {/* Individual Question */}
                    ...
                    <div style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{
                          ...
                          <div>
                            <strong>Jawaban:</strong> {item.correct_option?.toUpperCase()}
                          </div>
                        </div>
                        {item.explanation && (
                        ...
                        {item.attachment && (
                          <div style={{ marginTop: "1rem" }}>
                            ...
                            <div style={{ marginTop: "0.5rem" }}>
                              {item.attachment.match(/\.(jpg|jpeg|png|gif)$/i) && (
                                <img
                ...
                {item.type === "group" && (
                  <div>
                      ...
                      <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                          {(() => {
                            ...
                            const startOrder = children.length > 0 ? children[0].order_number : item.sortOrder;
                            const endOrder = children.length > 0 ? children[children.length - 1].order_number : startOrder;
                            return (
                              <h4 style={{ 
                                color: "#28a745", 
                                margin: "0 0 1rem 0",
                                ...
                                Grup Soal #{startOrder}{children.length > 1 ? `-${endOrder}` : ""}
                              </h4>
                            );
                          })()}
                          <div style={{ marginBottom: "1rem" }}>
                    ...
                    <div style={{ paddingLeft: "1rem" }}>
                      <h5 style={{ color: "#28a745", marginBottom: "1rem", fontWeight: "600" }}>
                        Subsoal dalam Grup:
                      </h5>
                      {getGroupChildren(item.id).map((sub, subIdx) => (
                        ...
                        >
                          ...
                          <div style={{
                            ...
                            <div><strong>A.</strong> {sub.option_a}</div>
                            <div><strong>B.</strong> {sub.option_b}</div>
                            <div><strong>C.</strong> {sub.option_c}</div>
                            <div><strong>D.</strong> {sub.option_d}</div>
                          </div>
                          <div style={{ 
                            fontSize: "0.9em",
                            display: "flex",
                            ...
                            <div>
                              <strong>Jawaban:</strong> {sub.correct_option?.toUpperCase()}
                            </div>
                          </div>
                          {sub.explanation && (
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            ...
            {(formMode === "create" || formMode === "edit") && !groupMode && (
              <div>
                <h3 style={{ 
                  ...
                  {getEditInfo() || (formMode === "edit" ? "Edit" : "Tambah")} Soal Individual
                </h3>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                    ...
                    <div>
                      ...
                      >
                        ...
                        {getAvailableOrderNumbers(form.id, false, formMode === "create").map((option) => (
                          <option 
                            key={option.value} 
                            value={option.value}
            ...
            {/* Group Form */}
            {(formMode === "create-group" || formMode === "edit-group") && groupMode && (
              <div style={{ maxHeight: "80vh", overflow: "auto" }}>
                <h3 style={{ 
                  ...
                  {getEditInfo() || (formMode === "edit-group" ? "Edit" : "Tambah")} Soal Grup
                </h3>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  ...
                  <div style={{
                    ...
                    <h4 style={{ color: "#28a745", marginTop: "0", fontWeight: "600" }}>Deskripsi Grup (Opsional)</h4>
                    ...
                    <div style={{ marginBottom: "1rem" }}>
                      ...
                      <select
                        ...
                        onChange={(e) => setForm({ ...form, group_order_number: parseInt(e.target.value) })}
                        required
                        style={{
                          width: "100%",
                          padding: "0.5rem",
                      ...
                      >
                        ...
                        {getAvailableOrderNumbers(form.id, true, formMode === "create-group").map((option) => (
                          <option 
                            key={option.value} 
                            value={option.value}
                            disabled={option.disabled}
                    ...
                    {form.attachment && (
                      <div style={{
                        ...
                        <p><strong>Preview Lampiran Grup:</strong></p>
                        {form.attachment.match(/\.(jpg|jpeg|png|gif)$/i) && (
                          <img
                            src={`http://localhost:8000${form.attachment}`}
                        ...
                        {form.attachment.match(/\.(mp3|wav|ogg)$/i) && (
                          <audio src={`http://localhost:8000${form.attachment}`} controls style={{ width: "100%" }} />
                        )}
                        {form.attachment.match(/\.(mp4|webm)$/i) && (
                          <video
                            src={`http://localhost:8000${form.attachment}`}
                  ...
                  {/* Children Section */}
                  <div>
                    <h4 style={{ color: "#28a745", fontWeight: "600" }}>Daftar Soal dalam Grup ({form.children?.length || 0} soal)</h4>

                    {form.children && form.children.map((child, idx) => (
                      <div
                        key={idx}
                        style={{
                          padding: "1rem",
                      ...
                      >
                        ...
                        {form.children.length > 1 && (
                          <div style={{ marginBottom: "1rem" }}>
                            ...
                            <select
                              value={idx}
                              onChange={(e) => moveSubsoalInGroup(idx, parseInt(e.target.value))}
                              style={{
                                width: "200px",
                                padding: "0.5rem",
                                border: "1px solid #ced4da",
                            ...
                            >
                              {getAvailableSubsoalOrders(form.id, idx).map((option) => (
                                <option 
                                  key={option.value} 
                                  value={option.value}
                                  disabled={option.disabled}
                        ...
                        <div style={{ marginBottom: "1rem" }}>
                          ...
                          <textarea
                            ...
                            onChange={(e) => updateChildData(idx, "question_text", e.target.value)}
                        ...
                        <div style={{
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateChildData(idx, "option_a", e.target.value)}
                              placeholder="Option A"
                              required
                              style={{
                                width: "100%",
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateChildData(idx, "option_b", e.target.value)}
                              placeholder="Option B"
                              required
                              style={{
                                width: "100%",
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateChildData(idx, "option_c", e.target.value)}
                              placeholder="Option C"
                              required
                              style={{
                                width: "100%",
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateChildData(idx, "option_d", e.target.value)}
                              placeholder="Option D"
                              required
                              style={{
                                width: "100%",
                        ...
                        <div style={{
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => updateChildData(idx, "correct_option", e.target.value)}
                              required
                              style={{
                                width: "100%",
                                padding: "0.5rem",
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) => updateChildData(idx, "explanation", e.target.value)}
                            placeholder="Penjelasan jawaban..."
                            rows={2}
                            style={{
                              width: "100%",
